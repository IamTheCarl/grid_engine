...
pub trait TerrainGenerator {
    ...
    fn populate_chunk(&self, chunk: &mut Chunk) -> TerrainGeneratorResult;
}
...
impl TerrainGenerator for AbstractFlatWorld {
    fn initialize_block_ids(&mut self, registry: &mut BlockRegistry) {
    ...
    fn populate_chunk(&self, chunk: &mut Chunk) -> TerrainGeneratorResult {
        ...
        if chunk_location.y < 0 {
            chunk.iter_ideal_mut(Chunk::range_all_blocks()).for_each(|block| *block = self.abstract_block);
        }
...
impl RAMWorld {
    ...
    pub fn new(block_registry: BlockRegistry) -> Box<RAMWorld> {
    ...
    pub fn add_generator(&mut self, mut generator: Box<dyn TerrainGenerator>) {
