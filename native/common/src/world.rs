...
// TODO could async be used to make this thing more efficient at fetching nodes?
pub struct ChunkIndex {
    file: File,
    root: u64,
    loaded_nodes: HashMap<u64, ChunkNode>,
}
...
impl ChunkIndex {
    pub fn initialize(mut file: File) -> Result<ChunkIndex> {
        ...
        if file_length == 0 {
            ...
            file.set_len(NODE_LENGTH)?;

            // Create the index, with no root node. We'll add that in a moment.
            let mut index = ChunkIndex { file, root: 0, loaded_nodes: HashMap::new() };
            let root = index.new_node(0)?;
            index.root = root;
    ...
    pub fn create_chunk(&mut self, x: i16, y: i16, z: i16, pointer: u64) -> Result<()> {
    ...
    fn new_node(&mut self, parent: u64) -> Result<u64> {
    ...
    fn get_node<F: FnOnce(&mut ChunkNode) -> Result<R>, R>(&mut self, pointer: u64, function: F) -> Result<R> {
...
impl ChunkNode {
    fn load(file: &File, address: u64) -> Result<ChunkNode> {
    ...
    fn get_pointer_set(&self, index: u8) -> (u64, u64) {
        let index = NODE_POINTERS_START + index as usize * 16;
        let key = &self.memory[index..index + 7];
        let address = &self.memory[index + 8..index + 15];
    ...
    fn set_pointer_set(&mut self, index: u8, key: u64, address: u64) {
        let key = key.to_le_bytes();
        let address = address.to_le_bytes();

        let index = NODE_POINTERS_START + index as usize * 16;
        self.memory[index..index + 7].clone_from_slice(&address);
        self.memory[index + 8..index + 15].clone_from_slice(&address);
    }

    fn add_chunk(&mut self, key: u64, address: u64) -> AddChunkResult {
        if !self.is_empty() {
            let min = self.get_pointer_set(self.index_min()).0;
            let max = self.get_pointer_set(self.index_min()).0;
            unimplemented!()
        } else {
            // We add to ourself no matter what here.
            let index_min = self.index_min();
            self.set_pointer_set(index_min, key, address);
            *self.index_min_mut() -= 1; // We decide to add it in the down direction.
...
mod test_fileformate {
    ...
    fn chunk_keys() {
        ...
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x0000), 0x0000000000000000);

        assert_eq!(ChunkIndex::create_chunk_key(0x8000, 0x0000, 0x0000), 0x0000800000000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x8000, 0x0000), 0x0000400000000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x8000), 0x0000200000000000);

        assert_eq!(ChunkIndex::create_chunk_key(0x4000, 0x0000, 0x0000), 0x0000100000000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x4000, 0x0000), 0x0000080000000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x4000), 0x0000040000000000);

        assert_eq!(ChunkIndex::create_chunk_key(0x2000, 0x0000, 0x0000), 0x0000020000000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x2000, 0x0000), 0x0000010000000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x2000), 0x0000008000000000);

        assert_eq!(ChunkIndex::create_chunk_key(0x1000, 0x0000, 0x0000), 0x0000004000000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x1000, 0x0000), 0x0000002000000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x1000), 0x0000001000000000);

        assert_eq!(ChunkIndex::create_chunk_key(0x0800, 0x0000, 0x0000), 0x0000000800000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0800, 0x0000), 0x0000000400000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x0800), 0x0000000200000000);

        assert_eq!(ChunkIndex::create_chunk_key(0x0400, 0x0000, 0x0000), 0x0000000100000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0400, 0x0000), 0x0000000080000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x0400), 0x0000000040000000);

        assert_eq!(ChunkIndex::create_chunk_key(0x0200, 0x0000, 0x0000), 0x0000000020000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0200, 0x0000), 0x0000000010000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x0200), 0x0000000008000000);

        assert_eq!(ChunkIndex::create_chunk_key(0x0100, 0x0000, 0x0000), 0x0000000004000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0100, 0x0000), 0x0000000002000000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x0100), 0x0000000001000000);

        assert_eq!(ChunkIndex::create_chunk_key(0x0080, 0x0000, 0x0000), 0x0000000000800000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0080, 0x0000), 0x0000000000400000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x0080), 0x0000000000200000);

        assert_eq!(ChunkIndex::create_chunk_key(0x0040, 0x0000, 0x0000), 0x0000000000100000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0040, 0x0000), 0x0000000000080000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x0040), 0x0000000000040000);

        assert_eq!(ChunkIndex::create_chunk_key(0x0020, 0x0000, 0x0000), 0x0000000000020000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0020, 0x0000), 0x0000000000010000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x0020), 0x0000000000008000);

        assert_eq!(ChunkIndex::create_chunk_key(0x0010, 0x0000, 0x0000), 0x0000000000004000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0010, 0x0000), 0x0000000000002000);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x0010), 0x0000000000001000);

        assert_eq!(ChunkIndex::create_chunk_key(0x0008, 0x0000, 0x0000), 0x0000000000000800);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0008, 0x0000), 0x0000000000000400);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x0008), 0x0000000000000200);

        assert_eq!(ChunkIndex::create_chunk_key(0x0004, 0x0000, 0x0000), 0x0000000000000100);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0004, 0x0000), 0x0000000000000080);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x0004), 0x0000000000000040);

        assert_eq!(ChunkIndex::create_chunk_key(0x0002, 0x0000, 0x0000), 0x0000000000000020);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0002, 0x0000), 0x0000000000000010);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x0002), 0x0000000000000008);

        assert_eq!(ChunkIndex::create_chunk_key(0x0001, 0x0000, 0x0000), 0x0000000000000004);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0001, 0x0000), 0x0000000000000002);
        assert_eq!(ChunkIndex::create_chunk_key(0x0000, 0x0000, 0x0001), 0x0000000000000001);
    }
}
