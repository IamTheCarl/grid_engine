...
impl ChunkData {
    ...
    pub fn create(x: i16, y: i16, z: i16) -> Box<ChunkData> {
        ...
        let mut chunk = unsafe {
            ...
            // We want the array to be filled with zeros, but we might as well clear everything else while we're at it.
            // Notice we use a 1. The function write_bytes is a lot smarter than memset. It figures out that 1 means one whole struct.
...
impl ChunkDiskStorage {
    ...
    pub fn get_chunk(&self, x: i16, y: i16, z: i16) -> Result<Option<Box<ChunkData>>> {
        let path = self.create_chunk_path(x, y, z);
...
mod test_fileformate {
    ...
    fn generate_chunk_file_names() {
        // This just verifies that we are creating file names from chunk keys. It doesn't go into the details of the keys.
        assert_eq!(
